...
/**
 * Copy templates/coding_prompt.md to .agent-harness/ directory
 * @returns True if copy was successful, false if file already exists
 * @throws FileOperationError if copy fails
 */
export function copyCodingPromptTemplate(): boolean {
  const sourcePath = path.join(process.cwd(), 'templates', 'coding_prompt.md');
  const destPath = path.join(process.cwd(), '.agent-harness', 'coding_prompt.md');
  
  // Ensure .agent-harness directory exists
  const agentHarnessDir = path.join(process.cwd(), '.agent-harness');
  if (!fs.existsSync(agentHarnessDir)) {
    try {
      fs.mkdirSync(agentHarnessDir, { recursive: true });
    } catch (error) {
      const friendlyMessage = getFriendlyErrorMessage(error as Error, 'creating the .agent-harness directory');
      throw new FileOperationError(friendlyMessage, (error as any)?.code, 'create_dir');
    }
  ...
  try {
    ...
    const friendlyMessage = getFriendlyErrorMessage(error as Error, 'copying coding_prompt.md template');
    throw new FileOperationError(friendlyMessage, (error as any)?.code, 'copy_file');
  }
}
